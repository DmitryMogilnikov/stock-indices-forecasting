...
async def health_check_service_route(response: Response):
    ...
    return HealthCheckResponse(status_code=status.HTTP_200_OK, status_message="Service is running.")
...
async def health_check_redis_route(response: Response):
    if redis_db.check_connection():
        response.status_code = status.HTTP_200_OK
        return HealthCheckResponse(status_code=response.status_code, status_message="Redis is connected.")

    response.status_code = status.HTTP_503_SERVICE_UNAVAILABLE
    return HealthCheckResponse(status_code=response.status_code, status_message="Redis is not connected.")
