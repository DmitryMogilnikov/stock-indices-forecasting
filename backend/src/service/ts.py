...
def get_days_to_reduce_procent(percentage_changes: List[float], initial_percent: float, target_reduction: float, tolerance: float = 1e-6) -> int:
    ...
    for percent_change in percentage_changes:
        days += 1
        if is_reduction_sufficient(initial_percent, percent_change, target_reduction, tolerance):
            break

    if days == len(percentage_changes) and not is_reduction_sufficient(initial_percent, percentage_changes[-1], target_reduction, tolerance):
        days = 0
